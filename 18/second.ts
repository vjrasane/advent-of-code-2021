import { get, isEqual, max, maxBy, set } from "lodash/fp";
...
const result = max(nums.map((first, index) => max(
	nums.filter((_, i) => i !== index).map(
		(second) => magnitude(reduce([first, second]))
	)
)))

// reduce((acc, curr) => { return reduce([acc, curr]) });
