...
import { get, isEqual, minBy, set, inRange, first, padCharsStart, sum, min, max } from "lodash/fp";
...
const evaluatePacket = (packet: Packet): number => {
	switch (packet.type) {
		case PacketType.Literal:
			return packet.value;
		case PacketType.Sum:
			return sum(packet.packets.map(evaluatePacket));
		case PacketType.Product:
			return packet.packets.reduce((acc, curr) => acc * evaluatePacket(curr), 1);
		case PacketType.Minimum:
			return min(packet.packets.map(evaluatePacket)) as number;
		case PacketType.Maximum:
			return max(packet.packets.map(evaluatePacket)) as number;
		case PacketType.LessThan: {
			const [first, second] = packet.packets.map(evaluatePacket);
